...
class ProxyStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        #
        # Iteration 0 scope: prove the whole path (device -> API key auth
        # -> API Gateway -> Lambda -> response) works end to end. Real
        # Bedrock integration — the structured-output contract described
        # in architecture.md — lands in iteration 1, once the model-tier
        # spike has picked a model. Deliberately not stubbed out with fake
        # Bedrock logic that would just be thrown away.
        proxy_fn = lambda_.Function(
            ...
            environment={
                # Nothing here yet. Iteration 1 adds the pinned Bedrock
                # model ID(s) here — server-side, so a compromised or
                # modified client can't switch to a more expensive model.
            },
        )
        # Least-privilege IAM: the default execution role CDK attaches
        # grants only CloudWatch Logs write. No Bedrock permission yet —
        # there's nothing to call. Iteration 1 adds bedrock:InvokeModel
        # scoped to the specific model ARN(s) the spike settles on, not
        # a wildcard. See architecture.md, "Security posture".
